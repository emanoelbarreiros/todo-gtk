...
import GI.Gtk.Declarative.App.Simple
    ( run,
      App(App, view, update, inputs, initialState),
      AppView,
      Transition(..) )
...
data Event = TodoTextChanged Text | TodoSubmitted | TodoToggled Int | Closed
    ...
    bin Gtk.Window [#title := "TodoGTK+", on #deleteEvent (const (True, Closed))]
        ...
        where
            ...
            todoItem i todo = bin Gtk.CheckButton [#active := completed todo, on #toggled (TodoToggled i)] 
                $ widget Gtk.Label [#label := completedMarkup todo, #useMarkup := True, #halign := Gtk.AlignStart]
                            where
                                completedMarkup todo
                                    | completed todo = "<s>" <> name todo <> "</s>"
                                    | otherwise = name todo
            newTodoForm = widget Gtk.Entry [ #text := currentText s,
...
update' s e =
    case e of
        ...
        TodoSubmitted -> let newTodo = Todo {name = currentText s, completed = False}
...
main :: IO ()
main = void $
    run App {view = view', update = update', inputs = [], initialState = State {todos = mempty, currentText = mempty}}
